...
function PaymentsPage() {
  ...
  const handleSubscribe = async () => {
    ...
    if (
      ...
      // We are currently on localhost and using the emulators.
      // Creating a payment and subscription is using the Firebase Extension from Stripe OR Gum Road as is now set up for Text Hacker
      // The extension is using Cloud Functions that are not installed in the emulator currently
      // Once Firebase allows mirroring of the extensions in the local emulator, we can test it locally in the emulator
      // If you want to test it locally, then switch the flag in the file
      // .env.local NEXT_PUBLIC_USE_FIREBASE_EMULATORS to false
      toast.error(
        "Creating a subscription with the emulators locally is not supported yet. Please set NEXT_PUBLIC_USE_FIREBASE_EMULATORS to false in the file .env.local",
        {
          duration: 8000,
        }
      );
